...
The dataset binding also records `evidence_class`, `origin`, `held_out`, and `audit_holdout`. Built-in repository suites are always public. `--dataset <path>` requires an explicit `--evidence-class`; the external file must contain an identical `evidence_class`, a consistent boolean `held_out`, and `audit_holdout=true` only for `audit-held-out`. The runner rejects attempts to relabel public cases as either private held-out class. Expected labels remain in the bound dataset for the independent grader but never enter producer requests.
...
Repository-visible cases are public regressions. Keep release-decision held-out and audit-held-out tasks outside the Skill repository and unavailable to the proposer. Run them through `scripts/run-sealed-matrix` with a frozen proposal validation.
